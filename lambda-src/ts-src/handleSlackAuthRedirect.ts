...
export async function handleSlackAuthRedirect(event: APIGatewayProxyEvent): Promise<APIGatewayProxyResult> {
  try {
    ...
    const slackClientId = await getSecretValue('AtlasBot', 'slackClientId');
    const slackClientSecret = await getSecretValue('AtlasBot', 'slackClientSecret');
    ...
    let successText = `Successfully installed atlasBot in workspace`;
    if(data.team?.name) {
      successText = `Successfully installed atlasBot in workspace ${data.team.name}`;
    }
    else if(data.enterprise?.name) {
      successText = `Successfully installed atlasBot in organisation ${data.enterprise.name}`;
    } 
